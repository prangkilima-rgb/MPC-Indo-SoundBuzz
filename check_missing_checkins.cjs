...
async function checkLastFm(username, apiKey, dateStr) {
    ...
    try {
        ...
        console.error("Error fetching for", username, e.message);
        return 0;
    }
}
      ...
      for (const u of data.users) {
          ...
          for (const date of datesToCheck) {
              if (!history.includes(date)) {
                  ...
                  if (totalTracks > 0) {
                      if (!missingUsers[date]) missingUsers[date] = [];
                      missingUsers[date].push({
                          appUsername: u.appUsername,
                          lastFmUsername: u.lastFmUsername || u.appUsername,
                          tracksFound: totalTracks
                      });
                      console.log(`Found missing check-in: ${u.appUsername} on ${date} (${totalTracks} tracks)`);
                  }
              }
          }
      }
